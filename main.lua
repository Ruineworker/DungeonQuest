    ...
    for i, v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
        for i = 0,RepeatCount do
            task.spawn(function()
                ...
                elseif elseif v:FindFirstChild("cooldown") and v.cooldown.Value then then
...
function Functions:Teleport(Cframe)
    ...
    repeat task.wait()
        if Character:FindFirstChild("HumanoidRootPart") and bodyPosition ~= nil and bodyGyro ~= nil then
            Character:PivotTo(CFrame.new(Cframe.p + Vector3.new(0, Settings.AutoFarm.Distance * 2, 0))* CFrame.Angles(math.rad(270), 0, 0))
...
Players.LocalPlayer.PlayerGui:WaitForChild("RetryVote").Changed:Connect(function(change)
    if Settings.Misc.AutoRetry == true then
...
if queue_on_teleport ~= nil then
    queue_on_teleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/Ruineworker/DungeonQuest/refs/heads/main/main.lua"))()')
end
